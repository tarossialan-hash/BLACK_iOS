...
function onLoginSuccess(username) {
    ...
    // onLoginSuccess também é o callback final do botão "Sincronizar" (ver
    // recarregar() -> AndroidApp.sync()) — o banco local (Room) já foi
    // atualizado a essa altura, mas SEM isto a tela continuava mostrando os
    // dados antigos: mostrarCategoria() usa esse cache em memória pra
    // renderizar instantâneo ao reabrir uma categoria já visitada na mesma
    // sessão, e nada aqui invalidava esse cache depois de sincronizar — só
    // um reinício completo do app (que zera as variáveis JS) resolvia.
    window._vodCache = {};
    window._seriesCache = {};
...
function onRecentMoviesLoaded(jsonString) {
    ...
    movies.slice(0, 15).forEach(m => {
        ...
        const badgeContainer = construirBadges(movieName, m.streamId);
...
// Streams que já falharam por decodificador incompatível neste aparelho
// (HEVC 10-bit — HDR10 ou Dolby Vision, ver window.onFalhaDecodificacao4k).
// Só dá pra saber depois de uma tentativa real de reprodução, então a tag
// HDR só aparece a partir da segunda vez que o cliente vir aquela capa —
// é uma limitação inerente (não tem como adivinhar antes de tentar), mas
// evita ele cair de novo no mesmo filme sem saber por quê.
const CHAVE_STREAMS_INCOMPATIVEIS = 'streams_hdr_incompativel';

function streamEhIncompativel(streamId) {
    if (!streamId) return false;
    try {
        const lista = JSON.parse(localStorage.getItem(CHAVE_STREAMS_INCOMPATIVEIS) || '[]');
        return lista.includes(String(streamId));
    } catch (e) { return false; }
}

function marcarStreamIncompativel(streamId) {
    if (!streamId) return;
    try {
        const lista = JSON.parse(localStorage.getItem(CHAVE_STREAMS_INCOMPATIVEIS) || '[]');
        const idStr = String(streamId);
        if (!lista.includes(idStr)) {
            lista.push(idStr);
            localStorage.setItem(CHAVE_STREAMS_INCOMPATIVEIS, JSON.stringify(lista));
        }
    } catch (e) {}
}

function construirBadges(name, streamId) {
    const upperName = name ? name.toUpperCase() : '';
    let badgesHtml = '';
    const eh4k = upperName.includes('4K');
    const ehIncompativel = streamEhIncompativel(streamId);
    if (eh4k && ehIncompativel) {
        // Um badge só (4K HDR) em vez de dois empilhados — mais limpo, e o
        // cliente já sabe de cara que é a versão que travou nesse aparelho.
        badgesHtml += '<div class="badge-4k" title="Já travou nesse aparelho — o app troca pra outra qualidade automaticamente">4K HDR</div>';
    } else if (eh4k) {
        badgesHtml += '<div class="badge-4k">4K</div>';
    } else if (ehIncompativel) {
        badgesHtml += '<div class="badge-hdr" title="Já travou nesse aparelho — o app troca pra outra qualidade automaticamente">HDR</div>';
    }
    // "LEG"/"LEGENDADO" cobre o caso comum; \bL\b pega o "L" isolado que
    // alguns títulos usam como abreviação (ex.: "Filme [L]" ou "Filme - L") —
    // com fronteira de palavra pra não confundir com o L dentro de qualquer
    // outra palavra do título.
    if (upperName.includes('LEG') || /\bL\b/.test(upperName)) badgesHtml += '<div class="badge-leg">LEG</div>';
...
// Em alguns controles remotos, segurar o botão físico gera dois keydown
// "reais" (sem repeat=true) bem próximos em vez de um só — cada um dispara
// seu próprio timer de pressão longa, e se os dois completarem em sequência
// (favorita, depois desfavorita de volta) o resultado final parece "não
// favoritou nada", mesmo a barra tendo completado visualmente. Trava
// qualquer novo toggle por 1s depois do último, então o segundo disparo
// (se vier) não desfaz o primeiro.
let _ultimoToggleFavoritoTs = 0;
...
function alternarFavoritoCanal(div, ch) {
    ...
    if (agora - _ultimoToggleFavoritoTs < 1000) return;
    _ultimoToggleFavoritoTs = agora;
...
// Cache em memória pro título+tipo já buscado nesta sessão — abrir o mesmo
// filme/série de novo (ex.: voltou da tela de detalhes e entrou outra vez)
// não repete as 4 requisições à TMDB. Não resolve a primeira chamada da
// sessão (DNS/TLS ainda "frios" nesse momento — ver o ajuste de DNS em
// Configurações pra isso), só evita refazer o trabalho depois.
const _tmdbCache = new Map();

async function fetchTmdbData(title, isSeries) {
    const cacheKey = (isSeries ? 'tv:' : 'movie:') + title;
    if (_tmdbCache.has(cacheKey)) return _tmdbCache.get(cacheKey);
    const resultado = await _fetchTmdbDataSemCache(title, isSeries);
    _tmdbCache.set(cacheKey, resultado);
    return resultado;
}

async function _fetchTmdbDataSemCache(title, isSeries) {
    try {
...
function playFullscreenVideo(streamUrl, title, idAtual, tipoAtual) {
    ...
    //
    // Guarda o contexto pra window.onFalhaDecodificacao4k saber o que
    // estava tocando quando o player nativo avisa erro de decodificador —
    // só filme entra na troca automática por qualidade menor (ver função).
    window._playingContext = { title: title || '', idAtual: idAtual ?? null, tipoAtual: tipoAtual || null };
    ...
    if (window.AndroidApp && window.AndroidApp.tocarVideoNativo) {
        pararPlayerAoVivo();
        window.AndroidApp.tocarVideoNativo(streamUrl, title || '', true);
        return;
    }
...
window.onNativeVideoEnded = function() {
    ...
    if (lista && atual != null && atual >= 0 && lista[atual + 1]) {
        tocarEpisodioDaLista(atual + 1);
        return;
    }
    // Filme, ou último episódio da série: não há próximo pra autoplay — sem
    // isso o player nativo fica preso na tela (última imagem congelada) até
    // o usuário apertar Voltar.
    if (window.AndroidApp && window.AndroidApp.pararVideoNativo) {
        window.AndroidApp.pararVideoNativo();
    }
};

window.onNativeVideoError = function(mensagem) {
    // Cai aqui também quando a própria versão alternativa (ver
    // onVersaoAlternativaEncontrada) falha — tentarFallbackSeFalhar=false
    // pra não entrar em loop, então o erro chega cru por esse caminho
    // genérico. Detecta pela assinatura do MediaCodec e troca por uma
    // mensagem que o cliente entende, em vez do texto técnico do Android.
    const pareceErroDeDecodificador = mensagem && (
        mensagem.indexOf('MediaCodecVideoRenderer') !== -1 ||
        mensagem.indexOf('format_supported=NO_EXCEEDS_CAPABILITIES') !== -1
    );
    if (pareceErroDeDecodificador) {
        avisoRapido('Vídeo não compatível com este dispositivo.');
        return;
    }
    avisoRapido('Erro ao reproduzir: ' + (mensagem || 'falha desconhecida'));
};

// Chamado pelo player nativo quando o decodificador do aparelho não sabe
// decodificar o formato (comum em Mi Stick e afins com 4K HDR10/Dolby
// Vision, que exigem HEVC 10-bit — muitos desses aparelhos só decodificam
// 8-bit). Filme entra na troca automática por uma versão em qualidade
// menor do mesmo título, já sincronizada no catálogo local; série não
// (buscar por nome podia trazer o episódio errado), só mostra o erro.
const MENSAGEM_VIDEO_INCOMPATIVEL = 'Vídeo não compatível com este dispositivo.';

window.onFalhaDecodificacao4k = function(titulo, mensagem) {
    const contexto = window._playingContext;

    // Marca já na primeira falha, mesmo que a troca automática dê certo
    // depois — essa capa específica (esse streamId) é a que não roda neste
    // aparelho, então a tag HDR deve continuar aparecendo nela.
    if (contexto && contexto.tipoAtual === 'movie' && contexto.idAtual) {
        marcarStreamIncompativel(contexto.idAtual);
    }

    if (!contexto || contexto.tipoAtual !== 'movie' || !window.AndroidApp || !window.AndroidApp.buscarVersaoAlternativa) {
        avisoRapido(MENSAGEM_VIDEO_INCOMPATIVEL);
        return;
    }
    let tituloLimpo = (titulo || contexto.title || '').replace(/\[.*?\]|\(.*?\)|\|.*?\|/g, ' ');
    tituloLimpo = tituloLimpo.replace(/\b(4K|FHD|UHD|1080p|720p|LEG|DUB|DUBLADO|LEGENDADO|CAM|TS|HDTS|TELESYNC)\b/gi, ' ').trim();
    if (!tituloLimpo) {
        avisoRapido(MENSAGEM_VIDEO_INCOMPATIVEL);
        return;
    }
    avisoRapido('Este aparelho não suporta a qualidade deste vídeo — procurando uma versão compatível...');
    window.AndroidApp.buscarVersaoAlternativa(tituloLimpo);
};

window.onVersaoAlternativaEncontrada = function(jsonString) {
    const contexto = window._playingContext;
    let resultados = [];
    try { resultados = JSON.parse(jsonString) || []; } catch (e) {}

    const candidato = resultados.find(r =>
        r.type === 'movie' && String(r.id) !== String(contexto && contexto.idAtual)
    );

    if (!candidato || !window.AndroidApp || !window.AndroidApp.getVodStreamUrl) {
        avisoRapido(MENSAGEM_VIDEO_INCOMPATIVEL);
        return;
    }

    const streamUrl = window.AndroidApp.getVodStreamUrl(candidato.id, candidato.containerExtension || 'mp4');
    if (!streamUrl) {
        avisoRapido(MENSAGEM_VIDEO_INCOMPATIVEL);
        return;
    }

    const tituloFinal = candidato.name || (contexto && contexto.title) || '';
    window._playingContext = { title: tituloFinal, idAtual: candidato.id, tipoAtual: 'movie' };
    // tentarFallbackSeFalhar = false: se essa versão também falhar, mostra
    // o erro em vez de tentar de novo (evita loop).
    window.AndroidApp.tocarVideoNativo(streamUrl, tituloFinal, false);
};
...
function tocarEpisodioDaLista(index) {
    ...
    playFullscreenVideo(streamUrl, ep.title || `Episódio ${ep.episodeNumber}`, ep.id || null, 'series');
}
...
function openVodModal(item, isSeries) {
    ...
    if (badgesSlot) { badgesSlot.innerHTML = construirBadges(item.name, isSeries ? null : item.streamId); }
    ...
    if (btnPlay) {
        btnPlay.onclick = () => {
            ...
            if (streamUrl) {
                const idAtual = isSeries ? (item.seriesId || item.streamId) : item.streamId;
                playFullscreenVideo(streamUrl, item.name, idAtual, isSeries ? 'series' : 'movie');
            }
        };
...
function onVodListLoaded(jsonString, fromCache) {
    ...
    renderizarGradeIncremental(container, movies, (m) => {
        ...
        let iconHtml = m.streamIcon ? `<img src="${otimizarCapa(m.streamIcon)}" class="vod-item-poster" loading="lazy" decoding="async" onerror="this.style.display='none'">` : '';
        const badgeContainer = construirBadges(m.name, m.streamId);
...
document.addEventListener('keydown', function(e) {
    if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
        ...
        const liveTvScreen = document.getElementById('live-tv-screen');
        const isLiveTvVisible = liveTvScreen && window.getComputedStyle(liveTvScreen).display !== 'none';
        const vodTvScreen = document.getElementById('vod-tv-screen');
        const isVodTvVisible = vodTvScreen && window.getComputedStyle(vodTvScreen).display !== 'none';
        ...
        } else if (isLiveTvVisible) {
            // Sem isto, cada tecla na TV ao vivo caía no fallback genérico
            // abaixo e varria o documento inteiro — com uma categoria de
            // 1200+ canais isso significava milhares de getBoundingClientRect
            // síncronos por tecla, e ainda competindo com a decodificação do
            // vídeo tocando ao fundo. Travava especialmente em aparelhos mais
            // fracos (Mi Stick e afins).
            focusables = Array.from(liveTvScreen.querySelectorAll('button, [tabindex="0"]'));
        } else if (isVodTvVisible) {
            // Mesmo motivo da TV ao vivo, só que para a grade de Filmes/
            // Séries (categoria pode ter milhares de títulos).
            focusables = Array.from(vodTvScreen.querySelectorAll('button, [tabindex="0"]'));
        } else {
